...
long GetSelSrcLineID(HMIXER hMixerDevice, unsigned int DstLineID)
{
	...
	for  (unsigned int iCtrl=0; iCtrl<mxl.cControls; iCtrl++)
	{
		...
		if (!ControlList[iCtrl].cMultipleItems)
			continue;
		mmres = GetControlDetailList(hMixerDevice, &mixList[0],  (HWND) ControlList[iCtrl].cMultipleItems, ControlList[iCtrl].dwControlID);
		if(mmres != MMSYSERR_NOERROR)	continue ;
		
		/* Put result in mixBoolean - a list of boolean values (Selected) */
		mmres = GetControlDetailList(hMixerDevice, &mixBoolean[0],  (HWND) ControlList[iCtrl].cMultipleItems, ControlList[iCtrl].dwControlID);
		if(mmres != MMSYSERR_NOERROR) continue ;
...
/*
	Set a list of mixer controls BOOLEAN details for a given control ID
*/
MMRESULT SetControlDetailList(HMIXER hmxobj, LPMIXERCONTROLDETAILS_BOOLEAN list, HWND cMultipleItems, unsigned int dwControlID)
{
	MMRESULT mmres;
	MIXERCONTROLDETAILS mixDetails;
	...
	mixDetails.cChannels = 1;
	mixDetails.hwndOwner = cMultipleItems;
	mixDetails.cbDetails = sizeof(MIXERCONTROLDETAILS_BOOLEAN);
	mixDetails.paDetails = list;
...
*/
MMRESULT SetControlDetailList(HMIXER hmxobj, LPMIXERCONTROLDETAILS_UNSIGNED list, HWND cMultipleItems, unsigned int dwControlID)
{
	MMRESULT mmres;
	MIXERCONTROLDETAILS mixDetails;
...
*/
MMRESULT GetControlDetailList(HMIXER hmxobj, LPMIXERCONTROLDETAILS_LISTTEXT list, HWND cMultipleItems, unsigned int dwControlID)
{
	MMRESULT mmres;
	MIXERCONTROLDETAILS mixDetails;
...
*/
MMRESULT GetControlDetailList(HMIXER hmxobj, LPMIXERCONTROLDETAILS_BOOLEAN list, HWND cMultipleItems, unsigned int dwControlID)
{
	MMRESULT mmres;
	MIXERCONTROLDETAILS mixDetails;
...
{
	return m_ArrayMixerDevice.GetSize();
}
...
CAudioInput::CMixerDevice::CMixerDevice(int index)
{
	...
	mmres = mixerGetDevCaps((unsigned int)m_hMixerDevice, (LPMIXERCAPS) &mixCaps,	sizeof(MIXERCAPS));
...
void CAudioInput::CMixerDevice::AdjustInputVolume(unsigned long SrcID)
{
	...
	int nLines = m_ArrayInputLine.GetSize();
...
int CAudioInput::CMixerDevice::CreateInputLineArray()
{
	...
	/* Get Mixer Device' capabilities */
	mmres = mixerGetDevCaps((unsigned int)hMixerDevice, (LPMIXERCAPS) &mixCaps,	sizeof(MIXERCAPS));
	...
	for (int i=0 ; i<nDestLines ; i++)
	{
		...
		for  (unsigned int iCtrl=0; iCtrl<mxl.cControls; iCtrl++)
		{
			...
			/* Put result in mixBoolean - a list of boolean values (Selected) */
			mmres = GetControlDetailList(m_hMixerDevice, &mixBoolean[0],  (HWND) ControlList[iCtrl].cMultipleItems, ControlList[iCtrl].dwControlID);
			if(mmres != MMSYSERR_NOERROR) continue  ;
	...
	return m_ArrayInputLine.GetSize();
}
...
int CAudioInput::CMixerDevice::CreatePhysicalDevArray()
{
	...
	return m_ArrayPhysicalDev.GetSize();
}
...
{
	CInputLine * il;
	unsigned int nLines = m_ArrayInputLine.GetSize();
	if (Index >= nLines)
		return false;
	...
	*iLine = -1;
	...
	for (int i=0 ; i<nDestLines ; i++)
	{
		...
		for  (unsigned int iCtrl=0; iCtrl<mxl.cControls; iCtrl++)
		{
			...
			/* Get list of sub-controls of the MUX control */
			mmres = GetControlDetailList(m_hMixerDevice, &mixList[0],  (HWND) ControlList[iCtrl].cMultipleItems, ControlList[iCtrl].dwControlID);
			if(mmres != MMSYSERR_NOERROR)	continue ;
...
int CAudioInput::CMixerDevice::SetSpeakers(bool restore, bool mute)
{
	...
	int count = m_ArrayPhysicalDev.GetSize();
...
CArray<  CAudioInput::CMixerDevice::CInputLine::sMuteLine , CAudioInput::CMixerDevice::CInputLine::sMuteLine> * CAudioInput::CMixerDevice::CInputLine::CreateArrayMuteCtrl(ULONG SrcType, const char * Name)
{
	...
	MIXERCAPS mixCaps;
	MMRESULT mmres = mixerGetDevCaps((unsigned int)m_hMixerDevice, (LPMIXERCAPS) &mixCaps,	sizeof(MIXERCAPS));
...
{	sMuteLine sm;
	int nMuteCtrls = m_ArrayMuteControl->GetSize();
