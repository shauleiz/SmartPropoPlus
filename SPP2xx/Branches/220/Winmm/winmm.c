...
__inline void _DebugChangeModPopUp(struct SharedDataBlock * DataBlock)
{
	char msg[4000];
	static int mod=-1, detect=-1, pos=-1;

	if (!gDebugLevel || !DataBlock)
		return;


	if (mod==DataBlock->ActiveModulation.iModType && detect==DataBlock->ActiveModulation.AutoDetectModShift && pos==DataBlock->ActiveModulation.ActiveModShift)
		return;

	sprintf(msg, "Former Modulation:\tMod=%d; Auto=%d; Pos=%d", mod, detect, pos);
	mod=DataBlock->ActiveModulation.iModType;
	detect=DataBlock->ActiveModulation.AutoDetectModShift;
	pos=DataBlock->ActiveModulation.ActiveModShift;
...
__inline void SetActiveProcessPulseFunction(struct SharedDataBlock * DataBlock)
{
	int set;

	if (DataBlock->ActiveModulation.AutoDetectModShift)
		set = 0;
	else if  (!DataBlock->ActiveModulation.ActiveModShift)
		set = 1;
	else
		set = 2;

	ProcessPulse = ListProcessPulseFunc[DataBlock->nModulations*set + DataBlock->ActiveModulation.iModType];
	if (!ProcessPulse)
		ProcessPulse = (PP)ProcessPulsePpm;

	_DebugChangeModPopUp(DataBlock);
}

/* 
	Start the SppConsole GUI application if it exists
*/
HANDLE StartSppConsole()
{
...
void StartPropo(void)
{
	...
	/* Start the SPP Console GUI application */
	StartSppConsole();
}
//---------------------------------------------------------------------------
