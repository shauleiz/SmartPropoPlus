...
int LoadProcessPulseFunctions(struct SharedDataBlock * target)
{
	...
	while(target->ModName[index] && index<MAX_MODS)
	{
		target->ListProcessPulseFunc = realloc(target->ListProcessPulseFunc, sizeof(void *)*(index+1));
		if (!strcmp(target->ModName[index], MOD_TYPE_PPM))
		{	/* Auto Detect? */
			if (target->ActiveModulation.AutoDetectModShift)
				target->ListProcessPulseFunc[index] = (void *)ProcessPulsePpm; /* AutoDetect PPM */
			else
			{
				if (target->ActiveModulation.ActiveModShift)
					target->ListProcessPulseFunc[index] = (void *)ProcessPulseJrPpm; /* Positive shift PPM */
				else
					target->ListProcessPulseFunc[index] = (void *)ProcessPulseFutabaPpm; /* Negative shift PPM */
			};
		}
		else if (!strcmp(target->ModName[index], MOD_TYPE_JR))
			target->ListProcessPulseFunc[index] = (void **)ProcessPulseJrPcm;
		else if (!strcmp(target->ModName[index], MOD_TYPE_FUT))
			target->ListProcessPulseFunc[index] = (void **)ProcessPulseFutabaPcm;
		else if (!strcmp(target->ModName[index], MOD_TYPE_AIR1))
			target->ListProcessPulseFunc[index] = (void **)ProcessPulseAirPcm1;
		else if (!strcmp(target->ModName[index], MOD_TYPE_AIR2))
			target->ListProcessPulseFunc[index] = (void **)ProcessPulseAirPcm2;
		index++;
	};


	target->ListProcessPulseFunc[index] = (void **)NULL;
	return index;
}

void SetActiveProcessPulseFunction(struct SharedDataBlock * DataBlock)
{
	ProcessPulse = (PP)DataBlock->ListProcessPulseFunc[DataBlock->ActiveModulation.iModType];
	if (!ProcessPulse)
...
void StartPropo(void)
{
	...
	DataBlock =  CreateSharedDataStruct(Modulation);
	...
	/* Mute the speakers */
	resout = pmixerGetNumDevs();
	resout = pmixerGetDevCapsA(0, &MixCaps,  sizeof(MIXERCAPS));
	mxl.cbStruct = sizeof(MIXERLINE);
	mxl.dwDestination = 0;
	mxl.dwSource = 0;
	resout = pmixerGetLineInfoA(0, &mxl,  MIXER_GETLINEINFOF_DESTINATION);
	mxl.dwDestination = 0;
	mxl.dwSource = 0;
	resout = pmixerGetLineInfoA(0, &mxl,  MIXER_OBJECTF_WAVEOUT);
	mxl.dwDestination = 1;
	mxl.dwSource = 0;
	resout = pmixerGetLineInfoA(0, &mxl,  MIXER_GETLINEINFOF_DESTINATION);
	mxl.dwDestination = 1;
