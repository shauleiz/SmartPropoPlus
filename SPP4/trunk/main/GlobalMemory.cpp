...
void SetFilterNamesToGlobalMemory(LPCTSTR * name)
{
	...
	pCurrentFilterName = (LPTSTR)gpSharedBlock + sizeof(struct SharedDataBlock) + total_length + 1;
...
far void * CreateSharedDataStruct(struct Modulations * data)
{
    ...
    if (dwMapErr != ERROR_ALREADY_EXISTS)
	{
		...
		{	/* Data from registry */
			...
			pCurrentModName = (LPTSTR)gpSharedBlock + sizeof(struct SharedDataBlock);
