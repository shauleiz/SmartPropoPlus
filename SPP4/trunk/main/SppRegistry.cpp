...
int CreateDefaultModRegistry()
{
	...
	{
		RegSetValueEx(hkMod, DefMods[nMod*2],0, REG_SZ, (const BYTE* )DefMods[nMod*2 +1], 1+(DWORD)_tcsclen(DefMods[nMod*2 +1]));
		nMod++;
	};

	/* Mark the default ACTIVE modulation (PPM) */
	RegSetValueEx(hkMod, MOD_ACTIVE ,0, REG_SZ, (const BYTE* )MOD_TYPE_PPM, 1+(DWORD)_tcsclen(MOD_TYPE_PPM));
...
LPWSTR GetCurrentMixerDeviceFromRegistry()
{
	...
	res = RegQueryValueExW(hkSpp, MIXER_DEV,  NULL, NULL, (LPBYTE)&(Active[0]),  &ValueDataSize);
	if (res != ERROR_SUCCESS)
		return NULL;

	RegCloseKey(hkSpp);
	return _tcsdup(Active);
}
...
LPWSTR GetCurrentEndpointDeviceFromRegistry()
{
	...
	/* Get  data */	
	ValueDataSize = MAX_VAL_NAME;
	res = RegQueryValueExW(hkSpp, ENDPOINT,  NULL, NULL, (unsigned char *)&(Active[0]),  &ValueDataSize);
	if (res != ERROR_SUCCESS)
		return NULL;
...
int GetCurrentInputLineFromRegistry(unsigned int *SrcID)
{
	...
	LPWSTR mdName = GetCurrentMixerDeviceFromRegistry();
	if (!mdName)
		return 0;
...
void SetCurrentMixerDeviceToRegistry(LPCWSTR MixerName)
{
	...
	/* Set Active entry */
	i = wcslen(MixerName);
	dd = 2*((DWORD)wcslen(MixerName)+1);

	RegSetValueExW(hkMixer, MIXER_DEV,0, REG_SZ, (const BYTE *)MixerName, 2*((DWORD)wcslen(MixerName)+1));
...
void SetCurrentEndpointDeviceToRegistry(LPCWSTR MixerName)
{
	...
	/* Set Active entry */
	RegSetValueExW(hkMixer, ENDPOINT,0, REG_SZ, (const BYTE *)MixerName, 2*(1+(DWORD)wcslen(MixerName)));
...
void SetCurrentInputLineToRegistry(LPCWSTR MixerName, unsigned int SrcID)
{
	...
	/* Set Active entry */
	RegSetValueExW(hkAudio, MixerName,0, REG_DWORD, (LPCBYTE)&SrcID, 4);
...
int GetDebugLevelFromRegistry(void)
{
	int res;
	HKEY hSpp;
	DWORD ValueDataSize, DebugLevel=0;
#ifdef REG_TEST
#include <stdio.h>
#include <time.h>
#include <assert.h>
...
struct Modulations * GetModulationFromRegistry(int Create)
{
	...
	for (i=0 ; i<nValues ; i++)
	{
		...
		if (_tcscmp(ValueName, MOD_ACTIVE))
		{ /* Not "Active"  - normal entry */
			Mod[index] = (struct Modulation *)malloc(sizeof(struct Modulation));
			if (!Mod[index])
				return NULL;
			Mod[index]->ModTypeDisplay = _tcsdup((LPCTSTR)ValueData);
			Mod[index]->ModTypeInternal = _tcsdup(ValueName);
			Mod[index]->index = index;
			if (!_tcscmp(Mod[index]->ModTypeInternal, &(Active[0])))
				iActive = index;
			index++;
		};
	};
...
int UpdateModRegistry()
{
	...
	while (DefMods[iMod*2])
	{
		res = RegQueryValueEx(hkResult, DefMods[iMod*2], NULL, NULL, NULL,  NULL);
		if (res != ERROR_SUCCESS)
			res = RegSetValueEx(hkResult, DefMods[iMod*2],0, REG_SZ, (const BYTE* )DefMods[iMod*2 +1], 1+(DWORD)_tcslen(DefMods[iMod*2 +1]));
		iMod++;
	};
...
int SetSelectedFilterNameToRegistry(LPCTSTR selected)
{
	...
	/* Insert default shift values */
	res =  RegSetValueEx(hkResult, SEL_FLTR ,0, REG_SZ, (const unsigned char *)selected, 1+(DWORD)_tcslen(selected));
	RegCloseKey(hkResult);
