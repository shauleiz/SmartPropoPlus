// SppAudio.cpp : Defines the class CSppAudio that enumerates all capture endpoints
//
...
////////////////////// Class CSppAudio //////////////////////
SPPINTERFACE_API CSppAudio::CSppAudio(HWND hWnd)
{
	/* Initializations */
	m_pEnumerator = NULL;
...
/*
	...
	Pass pointer to this CSppAudioobject as parent of the notification object
*/
{
	/* Initializations */
	m_pEnumerator = NULL;
	...
	if (FAILED(hr))
	{
		MessageBox(NULL, TEXT("WASAPI (CSppAudio): Could not create audio Endpoint enumerator \r\nStopping audio capture"), L"SmartPropoPlus Message" , MB_SYSTEMMODAL|MB_ICONERROR);
		EXIT_ON_ERROR(hr);
	};
	...
	if (FAILED(hr))
	{
		MessageBox(NULL,L"WASAPI (CSppAudio): Could not enumerate capture audio Endpoint\r\nStopping audio capture", L"SmartPropoPlus Message" , MB_SYSTEMMODAL|MB_ICONERROR);
		EXIT_ON_ERROR(hr);
	};
	...
	if (FAILED(hr))
	{
		MessageBox(NULL,L"WASAPI (CSppAudio): Could not count audio Endpoints\r\nStopping audio capture", L"SmartPropoPlus Message" , MB_SYSTEMMODAL|MB_ICONERROR);
		EXIT_ON_ERROR(hr);
	};
...
HRESULT	CSppAudio::Enumerate(void)
...
HRESULT CSppAudio::EnableAudioDevice(PVOID devID, bool Enable)
{
	IPolicyConfigVista *pPolicyConfigVista;
	IPolicyConfig *pPolicyConfig;
	ERole reserved = eConsole;
...
int		CSppAudio::CountCaptureDevices(void)
{		
	return (int)m_CaptureDevices.size();
}

HRESULT	CSppAudio::GetCaptureDeviceId(int nDevice, int *size, PVOID *Id)
{
...
HRESULT	CSppAudio::GetCaptureDeviceName(PVOID Id, LPWSTR * DeviceName)
...
bool	CSppAudio::IsCaptureDeviceActive(PVOID Id)
{
	// Given a pointer to the id (which uniquely identifies the capture device),
...
bool	CSppAudio::IsCaptureDevice(PVOID Id)
{
...
bool	CSppAudio::IsExternal(PVOID Id)
/*
...
SPPINTERFACE_API bool CSppAudio::IsDisconnected(PVOID Id)
{
...
SPPINTERFACE_API HRESULT CSppAudio::GetJackInfo(PVOID Id, KSJACK_DESCRIPTION *pJackDescData)
//-----------------------------------------------------------
...
bool	CSppAudio::IsCaptureDeviceDefault(PVOID Id)
{
// Given a pointer to the id (which uniquely identifies the capture device),
...
int 	CSppAudio::FindCaptureDevice(PVOID Id)
...
bool	CSppAudio::RemoveCaptureDevice(PVOID Id)
...
bool 	CSppAudio::ChangeStateCaptureDevice(PVOID Id, DWORD state)
...
bool 	CSppAudio::AddCaptureDevice(PVOID Id)
...

{
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_ADDDEV_IDNOTFOUND);
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_ADDDEV_STATE);
		LogMessageWithId(WARN, IDS_W_ADDDEV_STATE, Id,GetWasapiText(hr));
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_ADDDEV_PROP);
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_ADDDEV_FRND);
		LogMessageWithId(WARN, IDS_W_ADDDEV_FRND, Id,GetWasapiText(hr));
...
HRESULT	CSppAudio::RegisterNotification(void)
{
	HRESULT hr = S_OK;
	...
	if (FAILED(hr))
	{
		LogMessage(ERR, IDS_E_REGNOT_FAIL, GetWasapiText(hr));
...
*/
HRESULT	CSppAudio::DefaultDeviceChanged(EDataFlow flow, ERole role,LPCWSTR pwstrDeviceId)
...
HRESULT	CSppAudio::DeviceAdded(LPCWSTR pwstrDeviceId)
// Called (asynch) when device added
{
	// Send message to calling window indicating what happend
	if (m_hPrntWnd)
		PostMessage(m_hPrntWnd, WMSPP_AUDIO_ADD, (WPARAM)pwstrDeviceId, NULL);
	return S_OK;
}

HRESULT	CSppAudio::DeviceRemoved(LPCWSTR pwstrDeviceId)
// Called (asynch) when device removed
{
	// Send message to calling window indicating what happend
	if (m_hPrntWnd)
		PostMessage(m_hPrntWnd, WMSPP_AUDIO_REM, (WPARAM)pwstrDeviceId, NULL);
	return S_OK;
}

HRESULT	CSppAudio::DeviceStateChanged(LPCWSTR pwstrDeviceId, DWORD dwNewState)
...
HRESULT	CSppAudio::PropertyValueChanged( LPCWSTR pwstrDeviceId, const PROPERTYKEY key)
// Called (asynch) when device property changed
{
	// Send message to calling window indicating what happend
	if (m_hPrntWnd)
		PostMessage(m_hPrntWnd, WMSPP_AUDIO_PROP, (WPARAM)pwstrDeviceId, NULL);
	return S_OK;
}
...
*/
float CSppAudio::GetChannelPeak(PVOID Id, int iChannel)
...
// Non existing channel returns 0.0
{
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_CHPEAK_IDNOTFOUND);
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_CHPEAK_ACTCLNT);
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_CHPEAK_MXFRMT);
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_CHPEAK_NOINIT);
		LogMessageWithId(WARN, IDS_W_CHPEAK_NOINIT,Id, GetWasapiText(hr));
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_CHPEAK_ACTMTR);
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_CHPEAK_MTRCNT);
		LogMessageWithId(WARN, IDS_W_CHPEAK_MTRCNT,Id, GetWasapiText(hr));
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_CHPEAK_GETVAL);
		LogMessageWithId(WARN, IDS_W_CHPEAK_GETVAL,Id, GetWasapiText(hr));
...
double CSppAudio::GetDevicePeak(PVOID Id)
...
// Non existing endpoint returns 0.0
{
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_DEVPEAK_NOINIT);
		LogMessageWithId(WARN, IDS_W_DEVPEAK_NOINIT,Id, GetWasapiText(hr));
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_DEVPEAK_ACTMTR);
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_DEVPEAK_MTRCNT);
		LogMessageWithId(WARN, IDS_W_DEVPEAK_MTRCNT,Id, GetWasapiText(hr));
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_DEVPEAK_GETVAL);
		LogMessageWithId(WARN, IDS_W_DEVPEAK_GETVAL,Id, GetWasapiText(hr));
...
SPPINTERFACE_API double CSppAudio::GetLoudestDevice(PVOID * Id)
...
HRESULT CSppAudio::InitEndPoint(PVOID Id)
{
	HRESULT hr=S_OK;
	IMMDevice * pDevice = NULL;
	...
	{
		hr = E_FAIL;
		LogMessage(ERR, IDS_E_INITEP_NOID);
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_INITEP_IDNOTFOUND);
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_INITEP_ACTCLNT);
		LogMessageWithId(WARN, IDS_W_INITEP_ACTCLNT,Id, GetWasapiText(hr));
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_INITEP_MXFRMT);
	...
	if (FAILED(hr) &&  m_CurrentWaveFormat.wBitsPerSample == 8)
	{
		...
		if (FAILED(hr))
		{
			LogMessage(ERR, IDS_E_INITEP_FRMTSUP);
	...
	if (FAILED(hr))
	{
		LogMessage(WARN, IDS_W_INITEP_NOINIT);
		LogMessageWithId(WARN, IDS_W_INITEP_NOINIT,Id, GetWasapiText(hr));
	...
	if (FAILED(hr))
	{
		LogMessage(ERR, IDS_E_INITEP_EVTHND);
		LogMessageWithId(ERR, IDS_E_INITEP_EVTHND,Id, GetWasapiText(hr));
	...
	if (FAILED(hr))
	{
		LogMessage(ERR, IDS_E_INITEP_NOCAPT);
		LogMessageWithId(ERR, IDS_E_INITEP_NOCAPT,Id, GetWasapiText(hr));
...
HRESULT CSppAudio::StopCurrentStream(void)
{
	HRESULT hr=S_OK;
...
HRESULT CSppAudio::StartCurrentStream(void)
{
	HRESULT hr=S_OK;
	...
	if (FAILED(hr))
		LogMessage(WARN, IDS_W_STRTSTR_NOSTART, GetWasapiText(hr));
	...
	do { // Loop on buffer until empty - Usually only once
		...
		if (pPulseDataObj)
		{
			...
			if (FAILED(hr))
			{
				...
				LogMessage(ERR, IDS_E_PROCPACK_PRW, GetWasapiText(hr));
...
HRESULT CSppAudio::InitPulseDataObj(CPulseData * pPulseDataObj)
{
	...
	if (!FAILED(hr))
		LogMessage(INFO, IDS_I_INITPULSE);
		//LogStatus(INITPULSE,INFO,INITPLSOBJ,m_LogParam);
	else
		LogMessage(ERR, IDS_E_INITPULSE, GetWasapiText(hr));
		//LogStatus(INITPULSE,ERR,GetWasapiText(hr),m_LogParam);
	return hr;
}


SPPINTERFACE_API bool	CSppAudio::RegisterLog(LPVOID f, LPVOID param)
{
...
SPPINTERFACE_API bool	CSppAudio::RegisterAudioLog(LPVOID f, LPVOID param)
{
...
SPPINTERFACE_API bool	CSppAudio::RegisterProcessPulse(LPVOID f, LPVOID param)
{
...
void	CSppAudio::LogMessage(int Severity, int Code, LPCTSTR Msg)
{
	...
	PostMessage(m_hPrntWnd , WMSPP_LOG_AUDIO + Severity, (WPARAM)Code, (LPARAM)Msg);
}
