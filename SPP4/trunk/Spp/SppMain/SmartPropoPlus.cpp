...
void SppMessageBoxWithErrorCode(void)
{
	...
	_stprintf_s(msg, MAX_MSG_SIZE, _T("Error(%d): %s"), error_code, (LPCTSTR)lpMsgBuf);
...
void SetSelectedFilterIndex(const int i)
{
	...
	if (i>=0 && name && _tcslen(name) )
		SetSelectedFilterNameToRegistry(name);
	else
		SetSelectedFilterNameToRegistry(_T(""));
...
*/
int SetActiveMode(LPCTSTR selected)
{
...
LPCTSTR ModeDisplayFromInternalName(LPCTSTR internal)
{
	...
	while (DefMods[i*2])
	{
		if (!_tcscmp(DefMods[i*2], internal))
