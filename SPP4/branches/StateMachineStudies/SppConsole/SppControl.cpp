...
WORD Flags = FLG_NONE;
bool ReqToStop = false;
bool ReqToStart = false;
...
// Compute the conditions of the state
OperatState ComputeCondition(void)
{
	UINT PosQual=0;
	UINT JoyQual=0;
	...
	// Get the quality of the Position data - Higher than 75 is good quality
	PosQual = Spp->GetPositionDataQuality();

	// Get the quality of the joystick communication - Higher than 90 is good quality
...
void ComputeOperatState(void)
{
	...
	static OperatState PrevState = UNKNOWN;

	switch (OperatStateMachine)
	{		
	case UNKNOWN:
		/// The most general case - Try to figure out what to do
		PrevState = OperatStateMachine;
		// Request to stop received (ReqToStop)
		if (ReqToStop)
			OperatStateMachine =  S3;
		else
			OperatStateMachine =  ComputeCondition();
		break;
	case S0:
		// If started then go to state UNKNOWN
		// If request to start then reset request, Start processing and update conf file.
		PrevState = OperatStateMachine;
		if (Spp->IsCaptureActive())
			OperatStateMachine = UNKNOWN;
		if (ReqToStart)
		{
			ReqToStart = false;
	...
	case S12:
		PrevState = OperatStateMachine;
		// Signal is too weak -
		// The user sent SPP to sleep
		if (ReqToStop)
		{
			OperatStateMachine = S0;
			break;
		};
		// The signal improved - go to starting point
		if (GetAudioQuality(true)>LEVEL_LO)
			OperatStateMachine = UNKNOWN;
		break;
	case S3:
		// Reset request to stop processing, Stop Processing and update conf file.
		// If stopped then go to State S0

		if (PrevState != OperatStateMachine)
		{
			ReqToStop = false;
			Spp->Stop();
			Conf->Stopped(true);
		...
		if (!Spp->IsCaptureActive())
		{
			SendMessage(hDialog, WMSPP_PRCS_ALIVE, TRUE, 0);
			OperatStateMachine = S0;
		};

		break;

	// SPP is fully operational - any change will send it's state to UNKNOWN
	case W10:
	case W101:
	case W9:
	case W8:
	...
	case W82:
		...
		if (ComputeCondition() != OperatStateMachine)
			OperatStateMachine =  UNKNOWN;
		else if (PrevState != OperatStateMachine)
		{
			PrevState = OperatStateMachine;
			Spp->Start();
		};
	...
	// W7* are intermediary states - When the signal is not good they stop the processing
	case W7:
		PrevState = OperatStateMachine;
		if (Spp->IsCaptureActive())
			Spp->Stop();
		else
			OperatStateMachine =  S1;
		break;
	case W71:
		PrevState = OperatStateMachine;
		if (Spp->IsCaptureActive())
...
OperatState SetState(OperatState current, OperatState next, LPCTSTR Msg)
{
	...
	static OperatState tmp = UNKNOWN;

	LogMessage(INFO, next);
