...
int GetCurrentAudioStateFromRegistry()
{
	...
	if (!isBaseRegistryExist())
		CreateEmptyBaseRegistry();
...
/* Create an empty Audio2Joystick registry key */
int CreateEmptyBaseRegistry()
{
	LONG res;
	HKEY hkFms;

	res =  RegCreateKeyEx(HKEY_CURRENT_USER, REG_BASE, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_WRITE, NULL,&hkFms, NULL);		
	if (res != ERROR_SUCCESS)
	{
		SetLastError(res);
		return 0;
...
int GetWaveInParamsFromRegistry(int *nSamples, int *nBits, int *nBuffers, int *BufferSize)
{
	...
	res = RegQueryValueEx(hkAud, WAVEIN_NSAMPLES, NULL, NULL, (unsigned char *)nSamples,  &ValueDataSize);
	if (res != ERROR_SUCCESS)
		*nSamples = 0;
	else
		counter++;

	res = RegQueryValueEx(hkAud, WAVEIN_NBITS, NULL, NULL, (unsigned char *)nBits,  &ValueDataSize);
	if (res != ERROR_SUCCESS)
		*nBits = 0;
	else
		counter++;

	res = RegQueryValueEx(hkAud, WAVEIN_NBUFFERS, NULL, NULL, (unsigned char *)nBuffers,  &ValueDataSize);
	if (res != ERROR_SUCCESS)
		*nBuffers = 0;
	else
		counter++;

	res = RegQueryValueEx(hkAud, WAVEIN_BUFSIZE, NULL, NULL, (unsigned char *)BufferSize,  &ValueDataSize);
	if (res != ERROR_SUCCESS)
		*nBuffers = 0;
	else
		counter++;
...
BOOL GetLocationDialogWindowFromRegistry(int * x, int * y)
{
	...
	/* Get  data */	
	ValueDataSize=4;
	resx = RegQueryValueEx(hkSpp, LOCATIONX,  NULL, NULL, (unsigned char *)x,  &ValueDataSize);
	resy = RegQueryValueEx(hkSpp, LOCATIONY,  NULL, NULL, (unsigned char *)y,  &ValueDataSize);
	if (resy != ERROR_SUCCESS || resx != ERROR_SUCCESS)
		return FALSE;
...
void SetLocationDialogWindowToRegistry(int x,int y)
{
	...
	/* Insert default shift values */
	res =  RegSetValueEx(hkResult, LOCATIONX ,0, REG_DWORD, (const unsigned char *)&x, 4);
	res =  RegSetValueEx(hkResult, LOCATIONY ,0, REG_DWORD, (const unsigned char *)&y, 4);
	RegCloseKey(hkResult);
}
...
int GetAntiJitterStateFromRegistry(void)
{
	...
	/* Get  data */	
	ValueDataSize = 4;
	res = RegQueryValueEx(hkSpp, ANTI_JITTER,  NULL, NULL, (unsigned char *)&Active,  &ValueDataSize);
	if (res != ERROR_SUCCESS)
		return -1;
...
void SetAntiJitterStateToRegistry(int enable)
{
	...
	/* Insert default shift values */
	res =  RegSetValueEx(hkResult, ANTI_JITTER ,0, REG_BINARY, (const BYTE* )&enable, 4);
	RegCloseKey(hkResult);
}


/* Get the directory where SppConsole is located */
/* 
	Return:
		0:	If Registry entry does not exists.
		1:	If found
		-1:	If Registry entry exists but file is not there
		-2: General Error

	Parameters:
		Path:		Pointer to full path name of the executable. Valid when function returns 1 or -1. 
		PathSize:	Size of parameter 'Path' as allocated by caller. If actual size of 'Path' is bigger, return -2.
*/
int GetSppConsoleFullPathFromRegistry(char * Path, UINT PathSize)
{
	LONG res;
	HKEY hkSpp;
	char Active[MAX_PATH+1] = "";
	unsigned long ValueDataSize;
	HANDLE  hFile;
	WIN32_FIND_DATA FileData;
	TCHAR Buf[MAX_PATH], *pFilePart[MAX_PATH];
	DWORD nPath;
	...
	/* Open SPP  key for data query */
	res = RegOpenKeyEx(HKEY_LOCAL_MACHINE,REG_UNINST, 0, KEY_QUERY_VALUE , &hkSpp);
	if (res != ERROR_SUCCESS)
		return 0;

	/* Get  path from registry */	
	ValueDataSize = PathSize;
	res = RegQueryValueEx(hkSpp, UNINST_SPP,  NULL, NULL, (unsigned char *)&(Active[0]),  &ValueDataSize);
	if (res != ERROR_SUCCESS)
		return 0;

	/* Test length of path */
	if (ValueDataSize > PathSize)
		return -2;

	/* 
		'Active' now contains path to file 'UninstalSpp.exe' - 
		It is now converted to the path to the GUI
	*/
	nPath = GetFullPathName(Active, ValueDataSize, Buf, pFilePart);
	Active[nPath-strlen(pFilePart[0])] = '\0';
	strcat(Active,"SppConsole.exe");
	strcpy(Path, Active);
...
int GetFmsFullPathFromRegistry(char * Path, UINT PathSize)
{
	...
	/* Open SPP  key for data query */
	res = RegOpenKeyEx(HKEY_CURRENT_USER,REG_FMS, 0, KEY_QUERY_VALUE , &hkFms);
	if (res != ERROR_SUCCESS)
		return 0;

	/* Get  path from registry */	
	ValueDataSize = PathSize;
	res = RegQueryValueEx(hkFms, INSTPATH_FMS,  NULL, NULL, (unsigned char *)Path,  &ValueDataSize);
	if (res != ERROR_SUCCESS)
		return 0;
