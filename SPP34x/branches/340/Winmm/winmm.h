...
#ifdef _DEBUG
#define	N_WAVEIN_BUF	64 /* Debug only */
#else
#define	N_WAVEIN_BUF	64 /* 3.3.5 Vista compatability */
#endif

#ifdef _DEBUG
#define BASEPRODUCT	  "SPP3.4.0"
#else
...
*/
typedef struct _WaveInListStruct {
	pWaveInStruct	*pDevice;						// Array WaveInStruct's
...
//---------------------------------------------------------------------------
static WaveInListStruct * WaveInList;

static HWAVEIN       waveIn;         // WAVE IN
static WAVEFORMATEX  waveFmt;		 // WAVE FORMAT (IN)
static WAVEHDR      *waveBuf[N_WAVEIN_BUF];      // Array of poiters to audio buffers /* Version 3.3.5 - Vista compatability */
...
BOOL DoStartPropo;
char path[1024];
volatile static	struct SharedDataBlock * DataBlock;
PP ProcessPulse;
int gDebugLevel;
FILE * gCtrlLogFile, * gDataLogFile, * gChnlLogFile;
far void  ** ListProcessPulseFunc;
static int console_started;
static HANDLE hWaveInThread;
static HANDLE hMutexStopWaveIn;
static 	DWORD waveInThreadId;
static CRITICAL_SECTION RestartPropoCS;
