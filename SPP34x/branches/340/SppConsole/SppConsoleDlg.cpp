...
void CSppConsoleDlg::OnSelchangeMixerdevice() 
{
	...
	/****** 3.4.0 - Inform the DLL ******/
	if (!m_hPpJoyExDll)
		return; // SPP4FMS

	DLLFUNC1 pMixerChanged = (DLLFUNC1)GetProcAddress(m_hPpJoyExDll, "MixerChanged");
	if (!pMixerChanged)
	{	/* improved error reporting - 3.3.1 */
		DWORD errorcode = GetLastError();
		CString Msg;
		Msg.Format( "Cannot load function \"MixerChanged\" - error code:%d\nFile \"PPJoyEx.dll.exe\" is probably corrupted or too old", errorcode);
...
{
	DLLFUNC2 pStartPPJoyInterface, pStopPPJoyInterface;
	DWORD errorcode;
	CString Msg;
	...
	if (checked)
	{
		pStartPPJoyInterface = (DLLFUNC2)GetProcAddress(m_hPpJoyExDll, "StartPPJoyInterface");
		...
		(*pStartPPJoyInterface)(1); // TODO: Virtual Joystick is not necessarily '1'

		// Set timer number 1 to tick every second 
		m_Timer1 = SetTimer(1,200,(TIMERPROC)NULL);
	}
	else
	{
		pStopPPJoyInterface = (DLLFUNC2)GetProcAddress(m_hPpJoyExDll, "StopPPJoyInterface");
		...
		(*pStopPPJoyInterface)(1);// TODO: Virtual Joystick is not necessarily '1'

		// Kills the 1-second timer and clear the PPJoy status window
		KillTimer(m_Timer1);
	};
...
void CSppConsoleDlg::OnTimer(UINT nIDEvent)
{
	...
	// Test the status of the PPJoy Virtual Joystick 
	UINT res;
	DLLFUNC1 pTestPPJoy = (DLLFUNC1)GetProcAddress(m_hPpJoyExDll, "TestPPJoy");
	if (pTestPPJoy)
		res = (*pTestPPJoy)();
	
	// Get the current audio volume
	UINT Volume, Slider;
	DLLFUNC1 pGetAudioVolume = (DLLFUNC1)GetProcAddress(m_hPpJoyExDll, "GetAudioVolume");
	if (pGetAudioVolume)
		Volume = (*pGetAudioVolume)();
	else
		return;
	CAudioInput::CMixerDevice * md = m_AudioInput->GetMixerDevice(m_iSelMixer);
	if (md)
		Slider = md->GetInputLineVolumeValue(m_iSelLine);
	else
		return;
	if (Slider>=0)
		Volume = Volume*Slider/100;
