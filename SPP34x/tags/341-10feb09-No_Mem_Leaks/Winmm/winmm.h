...
#define	INITWINMM	if (!hWinmm) {LoadWinmm( __LINE__); TlsSetValue(TlsIndex, hWinmm);};
#define NAKED __declspec(naked)
#define VERYLONGWAIT	50
...
Message(const char * in_msg, UINT type ,const char * file_name,  int line_number);
int StartSppConsole();
