...
struct SharedDataBlock
{
	...
	/* Current audio mixer info: Text */
	char name_mixer[MAX_MIXER];			// Name of current audio mixer
	BOOL MixerChanged;					// Flag set by the GUI when filter has changed. Reset by the DLL
	BOOL AudioState;					// Audio State (Enable Audio input selection - Checkbox state)
