...
/*
	3.4.0
	Get the state of checkbox "Enable Audio input selection"
	Return:
	1  - Checked
	0  - Not checked
	-1 - No info
*/
int GetCurrentAudioStateFromGlobalMemory()
{
	int i;
...
void SetCurrentAudioStateToGlobalMemory(const int Active)
{
	if (!isGlobalMemoryExist())
		return;
	...
	gpSharedBlock->AudioState = Active;
...
char * GetCurrentMixerDeviceFromGlobalMemory(void)
{
	char * out;
	...
	out = strdup((char *)gpSharedBlock->name_mixer);
...
void SetCurrentMixerDeviceToGlobalMemory(const char * MixerName)
{
	if (!isGlobalMemoryExist())
		return;
	...
	strncpy(gpSharedBlock->name_mixer, MixerName,MAX_MIXER);
	gpSharedBlock->MixerChanged = TRUE;
...
far void * CreateSharedDataStruct(struct Modulations * data)
{
    ...
    if (dwMapErr != ERROR_ALREADY_EXISTS)
	{
		...
		gpSharedBlock->name_mixer[0] = '\0';		/* 3.4.0 */
		gpSharedBlock->MixerChanged = FALSE;		/* 3.4.0 */
		gpSharedBlock->AudioState = -1;				/* 3.4.0 */
