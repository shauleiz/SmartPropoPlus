...
CAudioInput::CAudioInput(HWND hDlg)
{
	...
	unsigned int nWaveInDev = waveInGetNumDevs();
	if (!nWaveInDev)
		return;

	// For every WaveIn device - get the corresponding mixer - if exists
	// If there is a corresponding mixer, add it to the array of mixer devices.
	for (unsigned int index=0; index<nWaveInDev ; index++)
	{
		mixer = new CMixerDevice(index, hDlg);
		if (mixer->isValid())
...
/*
	3.4.0
	Get the index of the mixer that the system considers as the Preferred Mixer Device
	This is done awkwardly like this:
	1. Open a WAVEIN stream for the Preferred Mixer Device (Mixer ID = WAVE_MAPPER) and get the Handle for this stream
	2. Get the mixer ID associated with the WAVEIN stream
	3. Close the WAVEIN stream
*/
int CAudioInput::GetPreferredMixerDeviceIndex(void)
{
...
//	WAVEINCAPS		caps;
	...
	/* Open the preferred device WAVEIN - no callback because we are not going to use it */
    mmres = waveInOpen(&waveIn, WAVE_MAPPER, &waveFmt, NULL, 0, CALLBACK_NULL);
	if (mmres!=MMSYSERR_NOERROR || !waveIn)
		return -1;

	/* Test which mixer device was selected */
	mmres = mixerGetID((HMIXEROBJ)waveIn, &MixerID, MIXER_OBJECTF_HWAVEIN);
	if (mmres != MMSYSERR_NOERROR)
		return -1;
...
int CAudioInput::GetMixerDeviceID(char *mixer)
{
	const char * MixerName;
...
CAudioInput::CMixerDevice::CMixerDevice(int index, HWND hDlg)
{
	unsigned int mmres;
	MIXERCAPS mixCaps;
	...
	// Open the mixer device that corresponds to Wave In device number 'm_WaveInID'
	// If sucsseeds then get the corresponding Mixer ID
	mmres = mixerOpen((LPHMIXER) &m_hMixerDevice, m_WaveInID, (DWORD)(__int64)hDlg, (DWORD)0, MIXER_OBJECTF_WAVEIN|CALLBACK_WINDOW);
	if(mmres != MMSYSERR_NOERROR) return;
	mmres = mixerGetID((HMIXEROBJ)m_hMixerDevice, &m_ID, MIXER_OBJECTF_HMIXER);
	if(mmres != MMSYSERR_NOERROR) return;
...
int CAudioInput::CMixerDevice::GetInputLineVolumeValue(int i)
{
	if (i >= m_ArrayInputLine.GetSize() || i<0)
		return -1;
	return m_ArrayInputLine.GetAt(i)->GetVolumeValue();
}
...
// Get the current volume value of the input line
// This value, in the range of 0-100 indicates the volume control position
// It does NOT indicate the actual input level of the signal
// Error indicated by negative volume values
int CAudioInput::CMixerDevice::CInputLine::GetVolumeValue(void)
{
	MIXERCONTROL ControlList[50]  = {0};
	long Volume;
	int cControls;
	...
	// Get the list of controls for this line
	mmres = GetLineControlList((HMIXEROBJ)m_hMixerDevice, m_ID, ControlList, &cControls);
	if (mmres != MMSYSERR_NOERROR)
		return -1;

	// Loop on all controls for this line until the volume value is found
	for (int i=0 ; i<cControls ; i++)
	{
		Valid = ::GetVolumeValue(m_hMixerDevice, ControlList[i].dwControlID, &Volume);
		if (Valid)
			return Volume;
	};

	// No volume control folund
	return -2;
}
...
bool CAudioInput::CMixerDevice::isValid(void)
{
	return m_ValidMixerDevice;
}

// Get the mixer ID
int CAudioInput::CMixerDevice::GetID(void)
{
	return m_ID;
}
