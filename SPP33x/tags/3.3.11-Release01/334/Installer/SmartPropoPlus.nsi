...
isGen:
	...
	StrCmp $FolderSPP4FMS $INSTDIR 0 PathGood
	MessageBox MB_OK|MB_ICONSTOP "Cannot install 'Generic SmartPropoPlus' in a folder $INSTDIR$\n\
	'SmartPropoPlus for FMS' is already installed in this folder$\n\
	Choose another folder"
	Abort

isFMS:
	ReadRegStr $FolderGenSpp  HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\SmartPropoPlus" "FolderGenSpp"
	StrCmp $FolderGenSpp $INSTDIR 0 PathGood
	MessageBox MB_OK|MB_ICONSTOP "Cannot install 'SmartPropoPlus for FMS' in a folder $INSTDIR$\n\
	'Generic SmartPropoPlus' is already installed in this folder$\n\
	Choose another folder"
	Abort
