...
int UpdateModRegistry()
{
	...
	while (DefMods[iMod*2])
	{
		res = RegQueryValueEx(hkResult, DefMods[iMod*2], NULL, NULL, NULL,  NULL);
		if (res != ERROR_SUCCESS)
			res = RegSetValueEx(hkResult, DefMods[iMod*2],0, REG_SZ, (const BYTE* )DefMods[iMod*2 +1], 1+strlen(DefMods[iMod*2 +1]));
		iMod++;
	};
...
int SetSelectedFilterNameToRegistry(const char * selected)
{
	...
	/* Insert default shift values */
	res =  RegSetValueEx(hkResult, SEL_FLTR ,0, REG_SZ, (const unsigned char *)selected, 1+strlen(selected));
	RegCloseKey(hkResult);
