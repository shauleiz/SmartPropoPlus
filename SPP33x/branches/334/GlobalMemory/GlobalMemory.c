...
char * GetCurrentMixerDeviceFromGlobalMemory(int * iMixer)
{
	...
	*iMixer = gpSharedBlock->iCurrentMixer;
	MixerName = gpSharedBlock->name_mixer;
...
far void * CreateSharedDataStruct(struct Modulations * data)
{
    ...
    if (dwMapErr != ERROR_ALREADY_EXISTS)
	{
		...
		gpSharedBlock->iCurrentMixer = WAVE_MAPPER;
		gpSharedBlock->name_mixer[0] = '\0';
