...
char * GetCurrentMixerDeviceFromRegistry(int * iMixer)
{
	LONG res;
	HKEY hkSpp;
	...
	/* Open SPP  key for data query */
	*iMixer = WAVE_MAPPER; // Default value
	res = RegOpenKeyEx(HKEY_CURRENT_USER,REG_SPP, 0, KEY_QUERY_VALUE , &hkSpp);
	if (res != ERROR_SUCCESS)
		return NULL;
	...
	if (iMixer) /* 3.3.4 - Pass the mixer index too */
	{
		ValueDataSize = 4;
		res = RegQueryValueEx(hkSpp, MIXER_DEV_I,  NULL, NULL, (unsigned char *)iMixer,  &ValueDataSize);
		if (res != ERROR_SUCCESS)
			*iMixer = WAVE_MAPPER;
	};
...
int GetCurrentInputLineFromRegistry(unsigned int *SrcID)
{
	...
	char * mdName = GetCurrentMixerDeviceFromRegistry(&iMixer);
	if (!mdName)
		return 0;
...
void SetCurrentMixerDeviceToRegistry(const char * MixerName, const int iMixer)
{
	LONG res;
	HKEY hkMixer;
	...
	RegSetValueEx(hkMixer, MIXER_DEV_I,0, REG_DWORD, (const unsigned char *)&iMixer, 4); /* 3.3.4 */
