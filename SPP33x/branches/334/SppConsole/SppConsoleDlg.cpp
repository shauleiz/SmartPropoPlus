...
static UINT NEAR WINMM_DEVICECHANGE;
...
BEGIN_MESSAGE_MAP(CSppConsoleDlg, CDialog)
	...
	ON_REGISTERED_MESSAGE(WINMM_DEVICECHANGE, OnDeviceChanged)
...
BOOL CSppConsoleDlg::OnInitDialog()
{
	...
	/* Register the message that some chage occured with the default mixer device */
	WINMM_DEVICECHANGE = RegisterWindowMessage(DEVICE_CHANGED); 
...
/*
	Handler to that is sent by the system every time some change in the devices has occured
	If the change is in the preferred device then used to update the GUI and then the DLL about it.
*/
LRESULT CSppConsoleDlg::OnDeviceChanged(WPARAM wParam, LPARAM lParam)
{
	return NULL;
}
...
void CSppConsoleDlg::PopulateAudioSource()
{
	...
	for (int i=0 ; i<nAudioDev ; i++)
		MixerList->InsertString(i, m_AudioInput->GetMixerDeviceName(i)); // Insert Mixer device name to combo box
...
int CSppConsoleDlg::GetCurrentMixerDevice()
{
	...
	if (iMixer<0) // -1 represents NO_DATA
...
void CSppConsoleDlg::SetCurrentMixerDevice(unsigned int iMixer)
{
	...
	::SetCurrentMixerDevice(MixerName, iMixer);
}
...
void CSppConsoleDlg::OnAudioPrint() 
{
	...
	if (ExitCode)
		MessageBox("Output file is: AudioStatus.txt\n It is on your Desktop"); /* Version 3.3.4 - Spelling error */
	else
