...
int CAudioInput::GetPreferredMixerDeviceIndex(void)
{
    ...
    DWORD mmres = waveInOpen(&waveIn, WAVE_MAPPER, &waveFmt, NULL, 0, CALLBACK_NULL);
...
CAudioInput::CMixerDevice::CMixerDevice(int index, HWND hDlg)
{
	...
	mmres = mixerOpen((LPHMIXER) &m_hMixerDevice, m_ID, (DWORD)hDlg, (DWORD)0, MIXER_OBJECTF_MIXER|CALLBACK_WINDOW);
	if(mmres != MMSYSERR_NOERROR)
		return;
