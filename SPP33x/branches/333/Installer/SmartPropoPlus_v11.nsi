...
;Interface Settings
  ...
  !define MUI_FINISHPAGE_RUN "$INSTDIR\SppConsole.exe"
...
; DLL Version manipulation
	!insertmacro GetLocalFileVer "..\winmm\Release\winmm.dll" "$VERSION" 	
...
; DLL Version manipulation
	!insertmacro GetLocalFileVer "..\winmm\Release_PPJoy\PPJoyEx.dll" "$VERSION" 	
...
Remove:
  DetailPrint "[I] Removing SPP4FMS files from $FolderSPP4FMS" ; Debug
; Remove files
...
ReplaceUtils:
	DetailPrint "[I] Moving Utility file shortcuts to $FolderGenSPP\Utilities" ; Debug
 	...
 	goto ReplaceCommon
DeleteUtils:
	DetailPrint "[I] Deleting Utilities shortcuts (SPP for FMS)" ; Debug
...
Remove:
  DetailPrint "[I] Removing GenSPP files from $FolderGenSpp" ; Debug
; Remove files
...
ReplaceUtils:
	DetailPrint "[I] Moving Utility file shortcuts to $FolderSPP4FMS\Utilities" ; Debug
 	...
 	goto ReplaceCommon
DeleteUtils:
	DetailPrint "[I] Deleting Utilities shortcuts (Generic)" ; Debug
	Delete "$SMPROGRAMS\SmartPropoPlus\SmartPropoPlus Utilities\*"
 	RMDir "$SMPROGRAMS\SmartPropoPlus\SmartPropoPlus Utilities"
ReplaceCommon:
...
isFMS:
  	...
  	call isFmsInstalled
  	IntCmp $FmsExist 1 NoNeedToRemind1
...
isGen:
	...
	StrLen $R2 $R0 
  	IntCmp $R2 0 PopAll
	DetailPrint "[MB] Installing Generic SPP. SPP for FMS is already installed"
...
isFMS:
	...
	StrLen $R2 $R0 
  	IntCmp $R2 0 PopAll
	DetailPrint "[MB] Installing SPP for FMS. Generic SPP is already installed"
	MessageBox MB_YESNO|MB_ICONEXCLAMATION|MB_DEFBUTTON2   "\
